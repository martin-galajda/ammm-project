...
def update_candidate_set(input, solution, candidate_set, new_element_solution):
  ...
  packages_to_be_checked = np.where(possible_pt[:,truck_updated] == 1)[0]
  ...
  for package in packages_to_be_checked:
    if package == p:
      continue
    possible_pt[package,truck_updated], solution["possible_pbl"][package, truck_updated] = can_be_assigned_to_truck(input, solution, package, truck_updated)
...
def get_element_loss_range(greedy_loss_function, input, solution, candidate_set):
  ...
  for possible_assignment_idx in range(len(possible_assignments[0])):
    ...
    precomputed_greedy_cost_package_truck[p,t] = greedy_loss_function(input, solution, (p, t))
...
def try_to_construct_solution(greedy_loss_function, alpha, input):
  # solution_set = set()
  ...
  while len(np.where(candidate_set == 1)[0]) > 0:
    min, max, precomputed_cost = get_element_loss_range(greedy_loss_function, input, solution, candidate_set)
...
def construct(greedy_loss_function, alpha, input):
  feasible_solution_obtained = False
  
  while not feasible_solution_obtained:
    solution = try_to_construct_solution(greedy_loss_function, alpha, input)
