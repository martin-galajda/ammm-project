...
default_greedy_cost_function_params = {
  'cost_already_assigned_truck': -400,
  'cost_for_possible_pbl_truck_assignment': 100,
  'cost_for_possible_truck_assignment': 0,
...
def greedy_cost_function(input, solution, potential_element_solution, candidate_set, parameters = default_greedy_cost_function_params):

  if parameters is None:
    parameters = default_greedy_cost_function_params
  ...
  number_of_possibilities_for_pbl = len(np.where(solution['possible_pbl'][p,:,:] == 1)[0])

  greedy_cost = int(already_assigned_truck) * parameters["cost_already_assigned_truck"]
  greedy_cost += number_of_possibilities_for_pbl * parameters["cost_for_possible_pbl_truck_assignment"]
  greedy_cost += (candidate_set[p,:].sum()) * parameters["cost_for_possible_truck_assignment"]
